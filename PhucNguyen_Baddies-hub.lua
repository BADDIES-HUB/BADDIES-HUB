...
-- NÚT BẬT TẮT (SỬA TEXT THÀNH PHUC)
...
-- === [ DÒNG CHỮ CẬP NHẬT TRÊN MENU CHÍNH ] ===
local UpdateLabelMain = Instance.new("TextLabel", Main)
UpdateLabelMain.Size = UDim2.new(1, 0, 0, 20); UpdateLabelMain.Position = UDim2.new(0, 0, 0, -25)
UpdateLabelMain.BackgroundTransparency = 1; UpdateLabelMain.TextColor3 = Color3.fromRGB(255, 255, 0)
UpdateLabelMain.Font = Enum.Font.FredokaOne; UpdateLabelMain.TextSize = 14
UpdateLabelMain.TextStrokeTransparency = 0
UpdateLabelMain.Text = "----CẬP NHẬT SCRIPT SAU 2 NGÀY----"

-- TIÊU ĐỀ SCRIPT (SỬA THÀNH PHUC NGUYEN X HUB)
...
-- === [ DÒNG CHỮ CẬP NHẬT TRÊN MENU TRACKER ] ===
local UpdateLabelSub1 = Instance.new("TextLabel", SubMenu)
UpdateLabelSub1.Size = UDim2.new(1, 0, 0, 20); UpdateLabelSub1.Position = UDim2.new(0, 0, 0, -25)
UpdateLabelSub1.BackgroundTransparency = 1; UpdateLabelSub1.TextColor3 = Color3.fromRGB(255, 255, 0)
UpdateLabelSub1.Font = Enum.Font.FredokaOne; UpdateLabelSub1.TextSize = 14
UpdateLabelSub1.TextStrokeTransparency = 0
UpdateLabelSub1.Text = "----CẬP NHẬT SCRIPT SAU 2 NGÀY----"
...
-- MENU PHỤ 2 (TOOLS MENU - BÊN TRÁI)
...
Instance.new("UICorner", SubMenu2); Instance.new("UIStroke", SubMenu2).Color = Color3.fromRGB(0, 200, 255)

-- === [ DÒNG CHỮ CẬP NHẬT TRÊN MENU TOOLS ] ===
local UpdateLabelSub2 = Instance.new("TextLabel", SubMenu2)
UpdateLabelSub2.Size = UDim2.new(1, 0, 0, 20); UpdateLabelSub2.Position = UDim2.new(0, 0, 0, -25)
UpdateLabelSub2.BackgroundTransparency = 1; UpdateLabelSub2.TextColor3 = Color3.fromRGB(255, 255, 0)
UpdateLabelSub2.Font = Enum.Font.FredokaOne; UpdateLabelSub2.TextSize = 14
UpdateLabelSub2.TextStrokeTransparency = 0
UpdateLabelSub2.Text = "----CẬP NHẬT SCRIPT SAU 2 NGÀY----"
...
-- NÚT MỞ MENU PHỤ 2 (TRONG MENU CHÍNH)
...
ToolsBtn.MouseButton1Click:Connect(function() 
    if Main.Visible then 
        ...
        SubMenu.Visible = false -- Tắt menu bên phải nếu đang mở
    end 
end)
    ...
    while task.wait() do
        if getgenv().TargetKill then
            ...
            if Target and Target.Character and Target.Character:FindFirstChild("Humanoid") and Target.Character:FindFirstChild("HumanoidRootPart") and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                -- CHỈ ĐẠP KHI MÁU < 2 VÀ CÒN SỐNG
                if TarHum.Health > 0 and TarHum.Health < 2 then 
                    KillStatus.Text = ">>> ĐANG DẬM (STOMPING)! <<<"
                    -- TP THẲNG VÀO NGƯỜI ĐỊCH ĐỂ ĐẠP
                    LocalPlayer.Character.HumanoidRootPart.CFrame = TarRoot.CFrame 
                    
                    VirtualUser:CaptureController()
                    keypress(Enum.KeyCode.E)
                    task.wait(0.1) -- Delay nhẹ để game nhận phím
                    keyrelease(Enum.KeyCode.E)
                else
                    -- LOGIC DI CHUYỂN MƯỢT (KHÔNG GIẬT)
                    if Dist > 8 then -- Nếu xa hơn 8 studs thì chạy tới
                        LocalPlayer.Character.Humanoid:MoveTo(TarRoot.Position)
                    else -- Nếu gần thì đi vòng quanh
                        Angle = Angle + 0.1 -- Tăng góc chậm hơn để bớt chóng mặt
