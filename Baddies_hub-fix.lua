-- [[ REDZ HUB CLASSIC V7.6 - FINAL STABLE & NOTIFICATIONS & AUTO KICK OLD ]] --
...
-- MENU CHÍNH
local Main = Instance.new("Frame", ScreenGui); Main.Size = UDim2.new(0, 280, 0, 300) -- Tăng chiều cao để chứa đồng hồ
...
-- ĐỒNG HỒ ĐẾM NGƯỢC (UPDATE TIMER) - NEW
local TimerLabel = Instance.new("TextLabel", Main); TimerLabel.Size = UDim2.new(1, 0, 0, 20); TimerLabel.Position = UDim2.new(0, 0, 0, -25)
TimerLabel.BackgroundTransparency = 1; TimerLabel.TextColor3 = Color3.fromRGB(255, 255, 0); TimerLabel.Font = Enum.Font.Code; TimerLabel.TextSize = 14
TimerLabel.TextStrokeTransparency = 0

-- LOGIC ĐỒNG HỒ
if not getgenv().StartTime then getgenv().StartTime = tick() end
local UpdateTime = 48 * 3600 -- 48 tiếng
spawn(function()
    while task.wait(1) do
        local TimePassed = tick() - getgenv().StartTime
        local TimeLeft = UpdateTime - TimePassed
        if TimeLeft > 0 then
            local h = math.floor(TimeLeft / 3600)
            local m = math.floor((TimeLeft % 3600) / 60)
            local s = math.floor(TimeLeft % 60)
            TimerLabel.Text = string.format("UPDATE IN: %02d:%02d:%02d", h, m, s)
        else
            TimerLabel.Text = "UPDATE AVAILABLE!"
...
-- TEXT GHI CHÚ
local NoteLabel = Instance.new("TextLabel", SubMenu2); NoteLabel.Size = UDim2.new(1, 0, 0, 30); NoteLabel.Position = UDim2.new(0, 0, 0.8, 0); NoteLabel.BackgroundTransparency = 1
NoteLabel.Text = "--- VUI LÒNG CHỜ 2 NGÀY ĐỂ UPDATE SCRIPT ---"; NoteLabel.TextColor3 = Color3.fromRGB(255, 100, 100); NoteLabel.Font = Enum.Font.SourceSansItalic; NoteLabel.TextSize = 11
