-- [[ PHUC NGUYEN X HUB - ULTIMATE EDITION ]] --
...
-- === [ 1. LOADING SCREEN (THEO YÊU CẦU) ] ===
local LoadScreen = Instance.new("ScreenGui", CoreGui); LoadScreen.Name = "PhucLoading"
local LoadFrame = Instance.new("Frame", LoadScreen); LoadFrame.Size = UDim2.new(1, 0, 1, 0); LoadFrame.BackgroundColor3 = Color3.fromRGB(10, 10, 15); LoadFrame.ZIndex = 999
local LoadTitle = Instance.new("TextLabel", LoadFrame); LoadTitle.Size = UDim2.new(1, 0, 0, 50); LoadTitle.Position = UDim2.new(0, 0, 0.4, 0); LoadTitle.Text = "PHUC NGUYEN X HUB"; LoadTitle.TextColor3 = Color3.fromRGB(255, 0, 0); LoadTitle.Font = Enum.Font.FredokaOne; LoadTitle.TextSize = 30; LoadTitle.BackgroundTransparency = 1
local LoadBarBG = Instance.new("Frame", LoadFrame); LoadBarBG.Size = UDim2.new(0.6, 0, 0, 10); LoadBarBG.Position = UDim2.new(0.2, 0, 0.5, 0); LoadBarBG.BackgroundColor3 = Color3.fromRGB(30, 30, 30); Instance.new("UICorner", LoadBarBG)
local LoadBar = Instance.new("Frame", LoadBarBG); LoadBar.Size = UDim2.new(0, 0, 1, 0); LoadBar.BackgroundColor3 = Color3.fromRGB(255, 0, 0); Instance.new("UICorner", LoadBar)
local LoadPercent = Instance.new("TextLabel", LoadFrame); LoadPercent.Size = UDim2.new(1, 0, 0, 30); LoadPercent.Position = UDim2.new(0, 0, 0.52, 0); LoadPercent.TextColor3 = Color3.new(1,1,1); LoadPercent.Font = Enum.Font.Code; LoadPercent.TextSize = 16; LoadPercent.BackgroundTransparency = 1

spawn(function()
    for i = 1, 100 do
        LoadBar:TweenSize(UDim2.new(i/100, 0, 1, 0), "Out", "Linear", 0.06, true)
        LoadPercent.Text = "Loading... " .. i .. "%"
        task.wait(0.06)
    end
    LoadPercent.Text = "LOADED SUCCESS!"
    task.wait(0.5)
    local TweenOut = TweenService:Create(LoadFrame, TweenInfo.new(1), {BackgroundTransparency = 1})
    TweenService:Create(LoadTitle, TweenInfo.new(1), {TextTransparency = 1}):Play()
    TweenService:Create(LoadBarBG, TweenInfo.new(1), {BackgroundTransparency = 1}):Play()
    TweenService:Create(LoadBar, TweenInfo.new(1), {BackgroundTransparency = 1}):Play()
    TweenService:Create(LoadPercent, TweenInfo.new(1), {TextTransparency = 1}):Play()
    TweenOut:Play()
    TweenOut.Completed:Connect(function() LoadScreen:Destroy() end)
end)

-- === [ 2. HÀM HỖ TRỢ (NEON & ANIMATION) ] ===
local function AddNeon(obj)
    local stroke = Instance.new("UIStroke", obj); stroke.Thickness = 2
    spawn(function()
        local h = 0
        while obj.Parent do
...
local function ToggleMenu(frame, visible)
    if visible then
        ...
        -- Lưu size gốc vào attribute để restore
        if not frame:GetAttribute("OrgSize") then frame:SetAttribute("OrgSize", frame.Size) end
        frame.Size = UDim2.new(0, 0, 0, 0)
        local TargetSize = frame:GetAttribute("OrgSize")
        
        local TweenIn = TweenService:Create(frame, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = TargetSize, BackgroundTransparency = 0})
        TweenIn:Play()
        for _, v in pairs(frame:GetDescendants()) do if v:IsA("GuiObject") then v.Visible = true end end
    else
...
-- NÚT ẨN GUI
local HideBtn = Instance.new("ImageButton", ScreenGui); HideBtn.Size = UDim2.new(0, 40, 0, 40); HideBtn.Position = UDim2.new(0.95, -50, 0.05, 0); HideBtn.BackgroundColor3 = Color3.fromRGB(0, 0, 0); HideBtn.BackgroundTransparency = 0.5; Instance.new("UICorner", HideBtn).CornerRadius = UDim.new(1, 0)
...
-- NÚT BẬT TẮT
local Toggle = Instance.new("TextButton", ScreenGui); Toggle.Size = UDim2.new(0, 50, 0, 50); Toggle.Position = UDim2.new(0.1, 0, 0.3, 0); Toggle.BackgroundColor3 = Color3.fromRGB(15, 15, 15); Toggle.Text = "PHUC"; Toggle.TextColor3 = Color3.fromRGB(255, 0, 0); Toggle.Font = Enum.Font.FredokaOne; Toggle.TextSize = 18; Instance.new("UICorner", Toggle)
AddNeon(Toggle) -- Neon cho nút bật tắt

-- MENU CHÍNH
local Main = Instance.new("Frame", ScreenGui); Main.Size = UDim2.new(0, 280, 0, 300); Main.Position = UDim2.new(0.5, -140, 0.5, -130); Main.BackgroundColor3 = Color3.fromRGB(20, 20, 20); Main.Visible = false; Instance.new("UICorner", Main)
AddNeon(Main) -- Neon cho menu chính

local UpdateLabelMain = Instance.new("TextLabel", Main); UpdateLabelMain.Size = UDim2.new(1, 0, 0, 20); UpdateLabelMain.Position = UDim2.new(0, 0, 0, -25); UpdateLabelMain.BackgroundTransparency = 1; UpdateLabelMain.TextColor3 = Color3.fromRGB(255, 255, 0); UpdateLabelMain.Font = Enum.Font.FredokaOne; UpdateLabelMain.TextSize = 14; UpdateLabelMain.Text = "----CẬP NHẬT SCRIPT SAU 2 NGÀY----"
...
local Container = Instance.new("ScrollingFrame", Main); Container.Size = UDim2.new(1, -10, 1, -40); Container.Position = UDim2.new(0, 5, 0, 35); Container.BackgroundTransparency = 1; Container.ScrollBarThickness = 2; Container.AutomaticCanvasSize = Enum.AutomaticSize.Y; local UIList = Instance.new("UIListLayout", Container); UIList.Padding = UDim.new(0, 5)

-- MENU PHỤ 1 (TRACKER)
local SubMenu = Instance.new("Frame", ScreenGui); SubMenu.Size = UDim2.new(0, 300, 0, 260); SubMenu.Position = UDim2.new(0.5, 150, 0.5, -130); SubMenu.BackgroundColor3 = Color3.fromRGB(25, 25, 25); SubMenu.Visible = false; Instance.new("UICorner", SubMenu)
AddNeon(SubMenu) -- Neon cho menu phụ 1

local UpdateLabelSub1 = Instance.new("TextLabel", SubMenu); UpdateLabelSub1.Size = UDim2.new(1, 0, 0, 20); UpdateLabelSub1.Position = UDim2.new(0, 0, 0, -25); UpdateLabelSub1.BackgroundTransparency = 1; UpdateLabelSub1.TextColor3 = Color3.fromRGB(255, 255, 0); UpdateLabelSub1.Font = Enum.Font.FredokaOne; UpdateLabelSub1.TextSize = 14; UpdateLabelSub1.Text = "----CẬP NHẬT SCRIPT SAU 2 NGÀY----"
...
-- MENU PHỤ 2 (TOOLS)
local SubMenu2 = Instance.new("Frame", ScreenGui); SubMenu2.Size = UDim2.new(0, 250, 0, 250); SubMenu2.Position = UDim2.new(0.5, -420, 0.5, -100); SubMenu2.BackgroundColor3 = Color3.fromRGB(30, 30, 35); SubMenu2.Visible = false; Instance.new("UICorner", SubMenu2)
AddNeon(SubMenu2) -- Neon cho menu phụ 2

local UpdateLabelSub2 = Instance.new("TextLabel", SubMenu2); UpdateLabelSub2.Size = UDim2.new(1, 0, 0, 20); UpdateLabelSub2.Position = UDim2.new(0, 0, 0, -25); UpdateLabelSub2.BackgroundTransparency = 1; UpdateLabelSub2.TextColor3 = Color3.fromRGB(255, 255, 0); UpdateLabelSub2.Font = Enum.Font.FredokaOne; UpdateLabelSub2.TextSize = 14; UpdateLabelSub2.Text = "----CẬP NHẬT SCRIPT SAU 2 NGÀY----"
...
DiscordBtn.Image = "rbxassetid://15079630733" -- Discord Logo
DiscordBtn.MouseButton1Click:Connect(function() setclipboard("https://discord.gg/GvswYacf"); Notify("Đã copy link Discord!") end)
local DiscordTxt = Instance.new("TextLabel", SubMenu2); DiscordTxt.Size = UDim2.new(0.7,0,0,30); DiscordTxt.Position = UDim2.new(0.25,0,0.85,0); DiscordTxt.BackgroundTransparency=1; DiscordTxt.Text="Copy Discord"; DiscordTxt.TextColor3=Color3.new(1,1,1); DiscordTxt.Font=Enum.Font.SourceSansBold; DiscordTxt.TextSize=12; DiscordTxt.TextXAlignment=0
...
ToolsBtn.MouseButton1Click:Connect(function() if Main.Visible then ToggleMenu(SubMenu2, not SubMenu2.Visible); if SubMenu2.Visible then ToggleMenu(SubMenu, false) end end end)

-- LOGIC KÉO THẢ (ĐẢM BẢO HOẠT ĐỘNG CHO CẢ 3 MENU & TOGGLE)
...
Toggle.MouseButton1Click:Connect(function()
    ...
    ToggleMenu(Main, target)
    if not target then ToggleMenu(SubMenu, false); ToggleMenu(SubMenu2, false) end
end)
